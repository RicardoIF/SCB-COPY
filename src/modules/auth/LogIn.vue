      ...
      <div class="login-content">
        ...
        <form onsubmit="return false;">
          <div class="control">
            ...
            <input class="light-border" id="txtID" name="txtID" type="text" value="" />
          </div>
          <div class="control">
            <label for="PIN_Contrase_a">PIN/Contrase&#241;a</label>
            <input class="light-border" id="txtUserPass" name="txtUserPass" type="password" />
          </div>
          <router-link class="router--link" :to="{name:'recover-pass'}">Olvide mi contrase&#241a?</router-link>
          <button @click="Authenticate" class="bg-black no-border button-square"><font-awesome-icon :icon="['fa', 'circle-arrow-right']" />
            Acceder
          </button>
        </form>
      </div>
